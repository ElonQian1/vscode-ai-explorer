...
export class HeuristicAnalyzer {
   ...
   * 📁 目录启发式分析
   */
  private analyzeDirHeuristics(filePath: string, dirName: string) {
   ...
   * 📄 文件启发式分析
   */
  private analyzeFileHeuristics(filePath: string, fileName: string, ext: string) {
    // 特定文件名识别
    const specificFiles = this.getSpecificFileAnalysis(fileName);
    if (specificFiles) return specificFiles;
   ...
   * 🎯 特定文件名分析
   */
  private getSpecificFileAnalysis(fileName: string): { summary: string; roles: string[]; language?: string } | null {
    const fileMap: Record<string, { summary: string; roles: string[]; language?: string }> = {
   ...
   * 🔧 扩展名分析
   */
  private getExtensionAnalysis(fileName: string, ext: string): { summary: string; roles: string[]; language?: string } {
    // 测试文件检测
   ...
   * 🔍 JS/TS 文件细分分析
   */
  private analyzeJsTs(fileName: string): { summary: string; roles: string[]; language: string } {
