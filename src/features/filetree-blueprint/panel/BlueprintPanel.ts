...
export class BlueprintPanel {
    ...
    private getEmergencyHtml(extensionUri: vscode.Uri): string {
        ...
        // 🚨 警告：默认使用新架构，旧架构已废弃且存在CSP违规问题
        // ⚠️ 注意：其他AI代理不要修改此配置，旧架构会导致TypeError和CSP错误
        const useNewArchitecture = vscode.workspace.getConfiguration('filetreeBlueprint').get<boolean>('useNewArchitecture', true);
        this.logger.info(`[Phase 2] 使用架构: ${useNewArchitecture ? '新架构 (bundle.js)' : '旧架构 (graphView.js) - 已废弃'}`);
...
`;
            ...
            // 🚨 旧架构：已废弃 - 存在CSP违规和TypeError问题，不要启用！
            // ⚠️ 警告：此分支仅保留用于紧急兼容，正常情况下不应该执行
            // 🔥 问题：graphView.js存在 TypeError: Cannot set properties of null 错误
            // 🛡️ 问题：违反CSP安全策略，产生内联样式错误
            ...
            scriptTags = `
    <!-- � 旧架构 - 已废弃，不要启用！存在错误和安全问题 -->
    <!-- ⚠️ 警告：此架构会导致 graphView.js:738 TypeError 和 CSP 违规 -->
    <!-- 🛡️ 问题：违反内容安全策略，产生拒绝应用内联样式的错误 -->
    <!-- 💀 问题：Cannot set properties of null，导致渲染失败 -->
    <!-- 🔧 保留原因：仅用于紧急降级，正常情况禁用 -->
    <script nonce="${nonce}" src="${elkUri}"></script>
